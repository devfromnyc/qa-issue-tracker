...
export function buildAssigneeOptions(board, usersById) {
  ...
  if (board.ownerId?.startsWith("guest_") && !seen.has(board.ownerId)) {
    options.push({
      id: board.ownerId,
      label: "Guest (board creator)",
      type: "user",
    });
  }
...
export function resolveAssigneeName(assigneeId, board, usersById) {
  ...
  if (assigneeId === board.ownerId && assigneeId.startsWith("guest_")) {
    return "Guest (board creator)";
  }
